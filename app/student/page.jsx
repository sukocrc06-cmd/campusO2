...
export default function StudentPanel() {
  ...
  const name = user?.user_metadata?.full_name || user?.email?.split("@")[0] || "Öğrenci";
    ...
    <div style={{ minHeight: "100dvh", background: "#f5f8fc", fontFamily: "system-ui, sans-serif", color: "#0f1b33" }}>
      ...
      <main style={{ width: "min(960px, 100%)", margin: "0 auto", padding: "28px 20px 60px" }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.12em", color: "#175cd3" }}>ÖĞRENCİ PANELİ</div>
          <h1 style={{ margin: "6px 0 0", fontSize: 28, letterSpacing: "-0.04em" }}>Merhaba, {name} 👋</h1>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
          <a href="/student/staj" style={{ textDecoration: "none", color: "inherit", padding: 24, borderRadius: 18, border: "1px solid #c7deff", background: "radial-gradient(280px 140px at 0% 0%, rgba(230,240,255,.9), transparent 70%), #fff", boxShadow: "0 18px 45px -28px rgba(15,43,90,.2)" }}>
            <div style={{ fontSize: 28, marginBottom: 10 }}>📋</div>
            <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.12em", color: "#175cd3" }}>VOL 2</div>
            <h2 style={{ margin: "6px 0 8px", fontSize: 18 }}>Staj Takip</h2>
            <p style={{ margin: 0, fontSize: 13, color: "#5b6b85", lineHeight: 1.5 }}>Başvuru oluştur, durumunu takip et</p>
          </a>

          <div style={{ padding: 24, borderRadius: 18, border: "1px dashed #e3ebf6", background: "#fff" }}>
            <div style={{ fontSize: 28, marginBottom: 10 }}>📱</div>
            <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.12em", color: "#8fa0bc" }}>VOL 1</div>
            <h2 style={{ margin: "6px 0 8px", fontSize: 18, color: "#8fa0bc" }}>QR Yoklama</h2>
            <p style={{ margin: 0, fontSize: 13, color: "#8fa0bc" }}>Yakında panel entegrasyonu</p>
          </div>
        </div>
      </main>
    </div>
