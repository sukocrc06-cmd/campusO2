  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#f5f8fc", fontFamily: "system-ui, sans-serif", color: "#0f1b33", display: "flex", flexDirection: "column" }}>
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 22px", borderBottom: "1px solid #e3ebf6", background: "#fff" }}>
        ...
        <a href="/signup" style={{ minHeight: 40, padding: "0 16px", fontSize: 13, textDecoration: "none", display: "inline-flex", alignItems: "center", borderRadius: 13, border: "1px solid #c7deff", color: "#0e4bae", fontWeight: 700, background: "rgba(255,255,255,0.75)" }}>Kayıt Ol</a>
      </header>
      ...
      <main style={{ flex: 1, display: "grid", placeItems: "center", padding: "40px 20px" }}>
        <div style={{ width: "min(420px, 100%)", background: "#fff", border: "1px solid #e3ebf6", borderRadius: 20, padding: "36px 32px", boxShadow: "0 18px 45px -28px rgba(15, 43, 90, 0.28)" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <h1 style={{ margin: "0 0 6px", fontSize: 24, letterSpacing: "-0.04em" }}>Giriş Yap</h1>
            <p style={{ margin: 0, color: "#5b6b85", fontSize: 14 }}>CampusO hesabınla devam et</p>
          </div>
          ...
          {message ? (
            <div style={{ marginBottom: 16, padding: "12px 14px", borderRadius: 12, background: "#effbf6", border: "1px solid #bde5d5", color: "#0b5c42", fontSize: 13, fontWeight: 600 }}>{message}</div>
          ) : null}
          ...
          <form onSubmit={handleLogin} style={{ display: "grid", gap: 14 }}>
            ...
            <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12, fontWeight: 700, color: "#5b6b85" }}>
              Şifre
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="••••••••" minLength={6} style={{ height: 46, padding: "0 14px", border: "1px solid #e3ebf6", borderRadius: 12, fontSize: 14, outline: "none" }} />
            </label>
          ...
          <p style={{ marginTop: 22, textAlign: "center", fontSize: 13, color: "#5b6b85" }}>
            Hesabın yok mu? <a href="/signup" style={{ color: "#175cd3", fontWeight: 700, textDecoration: "none" }}>Kayıt Ol</a>
          </p>
        </div>
      </main>
    </div>
