...
export default function AkademisyenStajPage() {
  ...
  async function handleOnayla(id) {
    const { error } = await supabase.from('stajlar').update({ onay_durumu: 'akademisyen_onayladi' }).eq('id', id);
    if (!error) {
      setStajlar(stajlar.map(s => s.id === id ? { ...s, onay_durumu: 'akademisyen_onayladi' } : s));
    } else {
  ...
  return (
    <div style={{ padding: '40px', fontFamily: 'sans-serif', maxWidth: '900px', margin: '0 auto' }}>
      <h1>👨‍🏫 Akademisyen Staj Onay Paneli (Vol 1-3)</h1>
      {loading ? <p>Yükleniyor...</p> : stajlar.length === 0 ? <p>Başvuru yok.</p> : (
              ...
              <tr key={staj.id}>
                ...
                <td style={{ padding: '10px', border: '1px solid #ddd' }}>
                  {staj.onay_durumu === 'beklemede' ? (
                    <button onClick={() => handleOnayla(staj.id)} style={{ background: '#28a745', color: '#fff', border: 'none', padding: '6px 12px', cursor: 'pointer' }}>Onayla</button>
                  ) : <span>Onaylandı</span>}
                </td>
              </tr>
            ))}
